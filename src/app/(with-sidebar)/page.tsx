...
import { getLastSearchCoords, CURRENT_LOCATION_LABEL, type Coords } from "@/lib/lastSearchCoords";
...
export default function HomePage() {
  ...
  async function resolveCoords(): Promise<Coords | null> {
    ...
    if (!coords) {
      search.setError("We need a location to search from. Try entering one, or allow location access.");
      return null;
    }
    return coords;
  }
  ...
  async function handleSearch(e: FormEvent) {
    ...
    try {
      ...
      await search.runSearch({
        ...
        lng: coords.lng,
        radiusMiles: radius,
        categoryPath: categoryPathForType(entityType),
        nameQuery: name,
      });
    } catch {
    ...
    <main className="flex min-h-screen flex-1 flex-col items-center bg-white px-6 pt-10 pb-16 dark:bg-black">
      <div className="flex w-full max-w-xl flex-col gap-10">
        <form onSubmit={handleSearch} className="flex flex-col gap-8">
          ...
          <TypeSelect value={entityType} onChange={setEntityType} />
